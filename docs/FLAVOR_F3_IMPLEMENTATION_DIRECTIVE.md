...
> | `--features flavor-live --lib` | **0** | **104**（T-8 の関所 H 配線で 105 → 104） | **0** |
> | `--features flavor-layer --all-targets` | **280** | — | **0** |
> | `--features flavor-live --all-targets` | **280** | — | **0** |
>
> **T-3 以降は `flavor-live` が基準となる。warnings は T-3〜T-7 が 105、T-8 の関所 H 配線以降は 104 である**（未使用経路が参照されたため 1 減）。差分で表明せよ。
>
> **【監査役の訂正 2・2026-07-29】** 本項は当初 `flavor-live --lib` の warnings を **12** としていたが、**12 は `flavor-layer` の値であり `flavor-live` のものではない。** `flavor-live` は `pocket-brain` を引くため `llm/` 配下が同時にコンパイルされ、実測は **105** である（T-8 以降は 104）。**監査役が feature 集合を取り違えたまま基準値を書いた。** これは clippy ゲートに関する 2 度目の誤りであり、同じ形（**測っていない構成の数字をゲートに書く**）を繰り返している。**基準値は必ず、ゲートが実際に走る構成で測れ。**
>
...
#### T-8 必須要件（T-5 監査で記録・2026-07-29）— 生成を sim ワーカー上で走らせるな

**T-5 は A-4 の半分しか担保していない。** ターン経路（`advance` → `kick_ambient_flavor`）からの同期生成は関所 G で構造的に塞がれたが、配信メッセージ `DeliverAmbientFlavor` の宛先は **sim ワーカー自身のキュー**（`flavor_tx = sender.clone()`）であり、`deliver_ambient_flavor` は**同じワーカースレッド上で走る**。

| 状態 | T-5 時点 |
|---|---|
| 今回のターンが生成を待つ | **解消済み**（関所 G が構造的に禁止） |
| **次のターンが生成を待つ** | **未解消** —— 生成中はワーカーが占有され、後続の `advance` / `submit_decision` がキューで待つ |

**実モデルを繋いだ瞬間、後者が顕在化する。** 現状これを止めるガードは無く、`deliver_ambient_flavor` の中で同期的にモデルを呼ぶのが最も自然な一手になっている。

**したがって T-8 では次を必須とする**:

1. `deliver_ambient_flavor` が**モデルを直接呼ばない**こと。生成は **`LlmHandle` の専用ワーカースレッドへ委譲**し、完了通知で `finish` を呼ぶ形にせよ（`LlamaContext` が `Send` でないため `LlmHandle` は既に専用スレッドを持っている —— 新しい並行機構を発明するな）
2. **関所 H（新設）**: `test_flv_i_14_delivery_never_runs_model_on_sim_worker` —— `deliver_ambient_flavor` の本体を**波括弧照合**で切り出し（関所 G と同じ作法）、`LlmHandle::generate` / `.generate(` / `flavor_gen::generate` を直接呼ばないことを表明する。**正の伴走**として、委譲先（handle への送出）が本体に存在することも表明せよ
3. **変異ドリル D-23**: `deliver_ambient_flavor` にモデル直呼びを戻し、**関所 H が RED になることを示せ**。復元して GREEN を示せ

> **「ターン内で待たない」と「ゲームが待たない」は別の命題である。** T-5 は前者を構造化した。**A-4 が要求しているのは後者である。**
