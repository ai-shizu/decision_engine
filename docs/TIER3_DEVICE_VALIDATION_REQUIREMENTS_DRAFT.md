...
## 14. G-4 後続（2026-07-31）— FE 貫通・コア健全性・出力退化の特定

**デバイス**: iPhone 17 Pro `Gggzns` / Release / PID 43458（FE 改修）・PID 46510（arena 計器）

### 14.1 FE 貫通 — **証明された**（§13.4 解決）

実機画面に **「※ 静かな空気。」** の描画を指揮官が目視確認。**「実機でフレーバが表示された」と書ける証拠がこれで初めて成立した。**

修正は呼び出し側のみ（`af74927`）。`take()` は `advance` 直後の単発で、生成完了は約 267ms 後、次ターンでは tick 不一致で破棄 —— **競合ではなく決定論的な取りこぼし**だった。有界ポーリング（12 回 × 200ms）が tick の内側で回るため、**FLV-I-13 の相関契約は一切緩めていない。** 対案（tick 不一致の破棄をやめる）は「表示されないバグ」を「終わったターンの文章が表示され得る危険」と交換するため採らなかった。

ポーリング本体は `src/lib/flavorPoll.ts` としてコンポーネントの外に出し、`tests-runtime/flavorPoll.test.ts` が 5 ケースを検証する（**CI で実際に走る**）。最重要は「無効化されたポールは `take` を撃たない」—— `take` は消費するため、撃ち続けると**次の tick の値を食い潰す。**

### 14.2 アリーナ終端計器 — **コアは健全**

**問題**: アリーナの診断は `eprintln!` であり、**Release では捨てられる**（G-0R 実証）。キャンペーンが死んだのか操作をやめただけなのかが**原理的に区別できなかった。** G-5 の取込・G-4 のフレーバと同型の穴。

`arena.sealed` / `arena.dead_reason` / `arena.turns_completed` を `log_model_u64` で送出（`f14cc10`）。実測:

```
21:18:39.337 arena.sealed=1
21:18:39.337 arena.dead_reason=0
21:18:39.337 arena.turns_completed=52
```

`CAMPAIGN_TICKS = TICKS_PER_QUARTER(13) × 4 = 52` ⇒ **完走。不変条件違反なし。**

**⇒ `SOVEREIGN · HARD STOP` バナー ＋ 入力ロックとは、キャンペーンが完走した姿である。** バナーは SovereignBar の**常設ラベル**（異常イベントではない）、ロックは封緘の帰結。

> **限界の明示**: 本実行は 52 で終わっており、**先行実行で報告された T18/T19 の停止は再現されていない。** 本データは「あの時も正常終了だった」ことを証明しない。**未再現・原因未確定として残す。** 計器は設置済みなので、再発すれば `dead_reason` が名指しする。

`dead_reason` 対応表: `0`=死亡なし / `1`=AccountingBreach / `2`=SnapshotDigestMismatch / `3`=ReplayDivergence / `4`=InternalInvariantBroken。**個別コードにしたのは、「死んだ」の 1 ビットでは*どの*不変条件が壊れたかをログが言えないため。**

### 14.3 出力退化の原因 — **二重のハードコード**

`discarded=0` はターン数の不足ではなかった。**出力空間を 1 点に潰す定数が 2 つ**あった。

| 定数 | 位置 | 効果 |
|---|---|---|
| `SlotValue::MoodCalm` | `handle.rs` の**プロンプト側と判定側の両方** | `render_prompt` が mood ラベルを逐語で埋めるため、**プロンプトが毎ターン同一バイト列** |
| **サンプラ seed `0`** | `handle.rs:514` → `service.rs:984` | 同一プロンプトから**同一の一手**しか引けない |

**偶然成立した対照実験:**

| | N | seed | 結果 |
|---|---:|---|---|
| **F-3（host・A-1 ハーネス）** | **52** | `i × 9973 + 1`（可変） | **discards 観測** |
| **本実行（実機・アリーナ）** | **52** | **`0` 固定** | **discards ゼロ** |

**同一 N・単一変数の差・正反対の結果。** 「20〜30 ターン回せば出る」という見込みが外れた理由は回数ではない。

指揮官が観測した **「52 ターン一度も変化しない同一文」** が、この退化の直接証拠である。**これは A-1-f と同型** —— `attempts` も `accepted` も `leaked` も健全に見え、経路は動き、出力も出る。**外から見て正常と区別がつかない。**

### 14.4 決定性契約の精査（seed 変更の可否）

**結論: ライブ経路の seed 変更は既存契約を破壊しない。** 根拠:

- **SPEC_FLAVOR_LAYER に seed / 決定性の契約は存在しない**（唯一の "seed" ヒットは無関係な `DeserializeSeed` の不採用記述）
- **呼び出し元が分離している。** `enqueue_flavor_generate_seeded` の本番呼び出しは `flavor_a1.rs`（ハーネス）のみ。`enqueue_flavor_generate`（seed 0）の本番呼び出しは `handle.rs:514`（アリーナ）のみ
- **A-1 deletability は seed 0 経路を通らない。** `scripts/flavor_a1_deletability.sh` は `flavor-a1-digest` バイナリを `--arm none/canned/live` で走らせ、その全アームがハーネスの**可変 seed** 経路を使う。digest 比較はアーム間の Decide-time 系列であり、アリーナの seed とは無関係
- **seed 0 に設計意図の記述がない。** `enqueue_flavor_generate` の doc は seed に一切触れない。一方 seeded 版の doc は **「T-8 discard-rate arms need independent draws」** と明記する —— **プロジェクトは「seed 固定なら同一の引き」を既に知っており、ライブ経路にだけ適用し忘れていた**

⇒ **変更は安全。ただし本節の記録をもって「調査済み」とし、変更自体は別コミットで行う。**

### 14.5 対処と残件

**対処済み（`3b1e7f8`）**: mood を `Session::market()` から導出（jump→Volatile / Stress→Tense / Calm→Calm / 未観測→Calm）。ホストテスト 2 件 —— **全分岐が到達可能**であること、および **3 つの mood が 3 つの異なるプロンプトを生む**こと（後者が無いと導出が装飾に終わり、退化が移動するだけになる）。

**残件**:
1. **ライブ経路の seed**（§14.4 で安全と判定済み・未変更）。**これを変えるまで盾の実弾射撃は原理的に成立しない**
2. **実機でのガード発砲**（`discarded > 0`）は依然未取得
3. **先行実行の T18/T19 停止**は未再現・原因未確定

> **本フェーズが繰り返し見つけた欠陥は「書かれているが、走っていない」だった。§13 の 2 件は「走っているが、届いていない」。§14.3 はその第三形態である —— 走っていて、届いていて、しかし常に同じものを届けている。**

---

## 15. 実機での盾の発砲（2026-07-31）— **Tier 3 完全完了**

**デバイス**: iPhone 17 Pro `Gggzns` / Release / PID 46904
**ビルド**: `d7ed6c0`（動的 mood ＋ 可変シード）/ 4 検査 ＋ 3 点 SHA ゲート通過済み
**操作**: INTERVIEW → `[ GD ]` → キャンペーン完走（52 ターン）

### 15.1 判定 — **`flavor.discarded = 2`。盾は実機で撃った**

```
flavor.attempts      = 49
flavor.accepted      = 47
flavor.discarded     = 2      ← 実機で初の発砲
flavor.unavailable   = 0
flavor.leaked        = 0
flavor.dropped_busy  = 3
arena.sealed=1  arena.dead_reason=0  arena.turns_completed=52
```

**F-3 が host で確立した FLV-R-12（指示文があってもモデルは数値を書き、ガードが撃墜する）が、実機の Metal 経路で直接観測された。** §13.3 以来の唯一の未取得証拠がこれで埋まった。

### 15.2 計器自身の健全性 — **保存則が成立している**

```
accepted 47 + discarded 2 + unavailable 0 = 49 = attempts        ✓
attempts 49 + dropped_busy 3              = 52 = turns_completed ✓
```

**52 回の advance が一つ残らず説明される。** 49 回が受理、3 回が busy 拒否。**計器が数え落としも二重計上もしていないことの、計器の外側からの裏付けである。**

> `dropped_busy` は §14 の時点で「承認範囲を少し越える追加」として入れたものだが、**この保存則を閉じているのはその 1 カウンタである。** これが無ければ `attempts=49` と `turns_completed=52` の差 3 は説明できず、「3 回どこかへ消えた」ようにしか読めなかった。

### 15.3 `leaked = 0` が初めて証明能力を持った

**§13.3 / §14 時点の `leaked=0` は空虚だった。** 撃墜対象が一度も飛来していない以上、ベルトが何も見つけないのは当然であり、ガードの能力について何も述べていなかった。

**本実行は違う。** ガードが 2 件を撃墜し、**通した 47 件は再スキャンでも全てクリーンだった。** すなわち二重の表明が成立した:

1. **ガードは実機で機能する**（2 件を撃墜）
2. **ガードが通したものは正しい**（47 件がベルトを通過）

### 15.4 非自明な観測 — 撃墜と生成時間の相関

**撃墜された 2 件は、いずれも生成時間が外れ値的に長い。**

| | admit → finish |
|---|---:|
| 典型的な受理サイクル | 約 250–270ms |
| **discard #1**（`attempts=5`） | **1,251ms** |
| **discard #2**（`attempts=19`） | **818ms** |

いずれも直前に `dropped_busy` が増えている（長い生成の最中に advance が来て弾かれた）。

**仮説（因果は未断定）: 長い出力ほどトークン数が多く、数値を書く機会が増える。** 49 サンプル中の 2 件が両方とも外れ値的に長いのは偶然にしては目を引く。**予算（`FLAVOR_MAX_TOKENS = 64`）の再検討や、長時間生成の打ち切りを考える際の入口になる観測である。**

### 15.5 アリーナ終端 — 正常完走の 2 回目の再現

`arena.sealed=1` / `dead_reason=0` / `turns_completed=52`。指揮官の UI 観測（`CAMPAIGN SEALED` ＋ TURN LOG 52）と一致。**§14.2 の結論が独立した 2 回目の実行で再現された。**

### 15.6 Tier 3 の最終状態 — **全ゲート GREEN・フェーズ完全完了**
...
| 残件 | 状態 |
|---|---|
| FE 貫通（§13.4） | **解決** —— 実機表示・市況連動で変化 |
| 出力退化（§14.3） | **解除** —— 2 定数を撤去（`3b1e7f8` / `d7ed6c0`） |
| **ガード実機発砲（§13.3）** | **達成 —— `discarded=2`（§15.1）** |
| コア健全性 | **確認** —— `dead_reason=0` / 52 完走（2 回再現） |

### 15.7 唯一の未確定事項

**先行実行の T18/T19 停止は依然として未再現。** 本実行も 52 完走であり、**「あれは正常終了だった」と書ける証拠は無い。** §14.2 の限界表明をそのまま維持する。計器（`arena.dead_reason`）は設置済みであり、**再発すれば不変条件の名前が直接ログに出る。** 閉じずに監視下へ置く。

### 15.8 本フェーズが見つけた欠陥の型

| 型 | 実例 |
|---|---|
| **書かれているが、走っていない** | `#[cfg(target_os="ios")]` 配下で一度も実行されず、走れば落ちるガード試験（`sanitize_one_liner`）／ 構造上ビルドできず一度も走れなかった CI ゲート ／ 呼出元ゼロのアクセサ |
| **走っているが、届いていない** | Release で捨てられる `eprintln!` 診断（取込・アリーナ終端）／ 生成の 267ms 前に単発で問い合わせて毎回取りこぼす FE |
