...
# Rev.7 (2026-07-12): 文書状態宣言を同期。E0〜E4 は実装・検証済み、E5 は 3000ms
#   計測ゲート未達のため未実装かつ着手禁止。設計本文・数式・定数・過去 Rev は変更なし
#   (Finding 6 / AI_SKILLS §12 と整合)。
# Rev.8 (2026-07-12): 読者向け前提命令を AI_SKILLS §0 の読み込みプロトコルへ同期。
#   状態宣言・設計本文・数式・定数は変更なし (Finding 7)。

> **読者への前提命令**: `docs/AI_SKILLS.md` §0 のルーティング表に従い、§1 と
> Target Echo 行の §12 を読め。その後、本 SPEC の変更対象に対応する節を読め。
> AI_SKILLS 全体の無条件な全文読了は要求しない。
> 不変条件については AI_SKILLS.md が正、凍結された設計・不変条件・E5 着手条件については本書が正。
>
...
>
> **実装状態の宣言**: Target Echo — **E0〜E4 は実装・検証済み。E5 は 3000ms 計測ゲート未達のため未実装かつ着手禁止。**
> E0〜E4 の現在状態の正本は実コードと `docs/AI_SKILLS.md` §12 の as-built。本 SPEC は凍結された設計・不変条件・E5 着手条件を保持する。完遂済みフェーズの再実装は禁止。dyad scope 等の宣言済み制約は E4 未完了を意味しない。
