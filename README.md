...
```
data/raw/
  diary.md, line_history.txt, calendar.json, finance.json, ai_consultations.json
data/knowledge/*.md              外部知識 (意思決定フレームワーク等)
data/processed/
  vectors.bin, metadata.json     DailyContext ベクトル (C++ とレイアウト同期)
  deep_profile.json, user_profile.json
  knowledge_vectors.bin
...
src/python/
  ...
  apple_calendar_sync.py         Apple カレンダー (macOS ローカル DB) 取込
  consultation_log.py            AI 相談履歴
  llm_config.py                  ローカル LLM 設定
  app.py                         CLI 相談 (旧来)

src/ui/
  app.py                         Textual TUI (4 タブ)
  calendar_widget.py, time_picker.py

src/cpp/search_engine.cpp        NEON SIMD 内積 + OpenMP Top-K

tools/llama-arm64/               llama-server (同梱想定・git 除外)
models/*.gguf                    ローカル LLM (git 除外)
...
python src\python\profiler.py              # --no-llm 可
...
- マージモード: **追記** / **上書き** (ICS・Apple 共通の Select)
...
- **ICS:** `src/python/calendar_sync.py` — 標準ライブラリのみでパース
- **Apple:** `src/python/apple_calendar_sync.py` — `~/Library/Calendars/` 等の SQLite (`CalendarItem` / `Event`)
...
- `data/raw/`・`data/processed/`・`models/` は `.gitignore` 対象 (個人データ・大容量モデル)
