...
import pipeline  # noqa: E402  (同ディレクトリ)

DIARY_MD = ROOT / "data" / "raw" / "diary.md"
LINE_HISTORY = ROOT / "data" / "raw" / "line_history.txt"
CALENDAR_JSON = ROOT / "data" / "raw" / "calendar.json"
AI_CONSULTATIONS_JSON = ROOT / "data" / "raw" / "ai_consultations.json"
FINANCE_JSON = ROOT / "data" / "raw" / "finance.json"
KNOWLEDGE_DIR = ROOT / "data" / "knowledge"
PROCESSED = ROOT / "data" / "processed"

DIARY_BIN = PROCESSED / "vectors.bin"
DIARY_META = PROCESSED / "metadata.json"
KNOWLEDGE_BIN = PROCESSED / "knowledge_vectors.bin"
KNOWLEDGE_META = PROCESSED / "knowledge_metadata.json"
DEEP_PROFILE = PROCESSED / "deep_profile.json"
USER_PROFILE = PROCESSED / "user_profile.json"
SEARCH_EXE = ROOT / "build" / "search_engine.exe"
...
# SETTINGS 手入力 (profiler が上書きしない固定情報)
FIXED_ATTRIBUTES = {
    "age": "",
    "gender": "",
    "height": "",
...
def load_user_profile() -> dict:
    """user_profile.json を読み込む (v2 + fixed_attributes)。"""
    empty = {
...
def save_fixed_attributes(attrs: dict) -> None:
    ...
    profile["fixed_attributes"].update(
        {k: str(v).strip() for k, v in attrs.items() if k in FIXED_ATTRIBUTES})
    USER_PROFILE.parent.mkdir(parents=True, exist_ok=True)
    USER_PROFILE.write_text(json.dumps(profile, ensure_ascii=False, indent=2),
                            encoding="utf-8")


def format_user_profile_summary() -> str:
    """SETTINGS 表示用: 自動推定プロフィールの読み取り専用サマリ。"""
    p = load_user_profile()
    ...
    if cats:
        ...
        if goals:
            lines.append(f"  言及された目標: {' / '.join(goals)}")
        lines.append("")
    ...
    if drives:
        ...
        for d in drives:
            w = d.get("weight", 0)
            lines.append(f"  - {d.get('drive', '?')} ({w:.0%}): {d.get('meaning', '')}")
        lines.append("")

    style = abstract.get("cognitive_style", {})
    if style.get("label"):
        lines += ["【認知スタイル】", f"  {style['label']}", ""]
        for b in style.get("biases", [])[:3]:
            lines.append(f"  · {b.get('name')}: {b.get('abstract_pattern', '')}")
    ...
    if tensions:
        ...
        for t in tensions[:3]:
            if isinstance(t, dict):
                lines.append(f"  - {t.get('tension', t)}")
            else:
                lines.append(f"  - {t}")
    ...
    if values:
        ...
        for v in values[:5]:
            lines.append(
                f"  {v.get('rank', '?')}. {v.get('value')} → {v.get('root_need')} ({v.get('weight', 0):.0%})")

    biases = auto.get("dominant_biases", [])
    if biases:
        lines += ["", "【認知バイアス】"]
        for b in biases:
            lines.append(f"  - {b.get('bias')} (強度 {b.get('intensity', 0):.0%})")

    llm = inferred.get("llm_deep_synthesis", "").strip()
    if llm:
