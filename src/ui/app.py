...
import apple_calendar_sync  # noqa: E402
import calendar_manager as cal  # noqa: E402
import calendar_sync  # noqa: E402
import consultation_engine as ce  # noqa: E402
import finance_manager as fin  # noqa: E402
from calendar_widget import CalendarWidget  # noqa: E402
from time_picker import TimePicker  # noqa: E402

DIARY_MD = ce.DIARY_MD
LINE_HISTORY = ce.LINE_HISTORY
USER_PROFILE = ce.USER_PROFILE
PROFILER_PY = ROOT / "src" / "python" / "profiler.py"
FIXED_FIELDS = ce.FIXED_ATTRIBUTE_FIELDS
...
class DecisionDashboard(App):
    ...
    def __init__(self):
        super().__init__()
        self.engine = ce.ConsultationEngine()
    ...
    def _load_fixed_attributes(self) -> None:
        attrs = ce.load_user_profile().get("fixed_attributes", {})
        for key, _ in FIXED_FIELDS:
            self.query_one(f"#fixed-{key}", Input).value = str(attrs.get(key, ""))

    def _load_profile_summary(self) -> None:
        self.query_one("#profile-summary", TextArea).text = ce.format_user_profile_summary()
    ...
    def _load_record_worker(self, date_str: str) -> None:
        ...
        self.call_from_thread(
            self._apply_record_data, date_str, events, transactions, diary)
    ...
    def _save_record(self) -> None:
        ...
        cal.set_events_for_date(d, self._current_events)
        fin.set_transactions_for_date(d, self._current_transactions)
        if body:
            cal.save_diary_for_date(d, body)
        elif cal.load_diary_for_date(d):
            cal.save_diary_for_date(d, "")

        self._refresh_calendar_marks()
        self._status("record-status", f"{d} を保存しました — 裏でDailyContext結晶化+ベクトル同期中…")
        self._sync_vectors()

    @work(thread=True, exclusive=True, group="sync")
    def _sync_vectors(self) -> None:
        try:
            rebuilt = self.engine.sync_diary_index(force=True)
            msg = "DailyContext結晶化+ベクトル同期完了" if rebuilt else "同期不要 (最新)"
        except Exception as e:
            msg = f"同期エラー: {type(e).__name__}: {e}"
        self.call_from_thread(self._status, "record-status", msg)
    ...
    def _sync_ics_worker(self, mode: str) -> None:
        try:
            ...
            summary = calendar_sync.sync_from_ics(path, mode=mode)
            self.call_from_thread(
    ...
    def _save_fixed_attrs(self) -> None:
        attrs = {key: self.query_one(f"#fixed-{key}", Input).value.strip()
                 for key, _ in FIXED_FIELDS}
        ce.save_fixed_attributes(attrs)
